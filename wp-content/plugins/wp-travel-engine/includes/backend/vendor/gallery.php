...
*/
  ...
  function wpte_gallery_meta_callback($post) {
    ...
    <table class='form-table'>
      <tr><td>
        <label for='wpte_gallery_id[enable]'><?php _e('Enable Gallery','wpte-trip-gallery'); ?></label>
        ...
        <div class="settings-note"><?php _e('Check this to enable gallery instead of featured image in single trip.','wpte-trip-gallery');?></div>
      </td></tr>
      <tr><td>
        <a class='feat-img-gallery-add button' href='#' data-uploader-title='Add image(s) to gallery' data-uploader-button-text='Add image(s)'><?php _e('Add image(s)','wpte-trip-gallery');?></a>
        ...
        <?php if ($ids) : unset($ids['enable']); foreach ($ids as $key => $value) : $image = wp_get_attachment_image_src($value); ?>
          <li>
            ...
            <small><a class='remove-image' href='#'><?php _e('Remove image','wpte-trip-gallery');?></a></small>
          </li>
