...
?>
	<div class="place-order-form-secondary-wrapper">
		...
		<div class="wp-travel-engine-order-form-wrapper">
			...
			<div class="wp-travel-engine-order-right-column">
				...
				<ul class="trip-property">
					...
					<li class="cart-trip-total-price"><span><?php _e('Total: ','wp-travel-engine');?></span><?php echo esc_attr($currency).'<span class="cart-trip-total-price-holder">'.esc_attr( $obj->wp_travel_engine_price_format( $tcost ) ).'</span>'.esc_attr( ' '.$code );?></li>
				</ul>
			</div>
		</div>
	    	...
	    	if(isset($pid)):
	    	    ...
	    	    if(isset($cost) && $cost!='')
	    	    {
	    	        ...
	    	        <div class="person-price-table">
	    	        	...
	    	        	<td><?php echo esc_attr($currency).'<span class="total-trip-price">'.$tcost.'</span>'.' '.esc_attr($code);?></td>
	    	            </tr>
	    	            </tbody>
	    	            </table>
						<?php // do_action( 'wte_update_cart' ); 
		...
		$trip_id = $pid;
		$_SESSION = $_POST;
		if(isset($wp_travel_engine_settings['advance']) && $wp_travel_engine_settings['advance']!='' )
		{ 
			$advance = ( $wp_travel_engine_settings['advance']/100 ) * $tcost;
			$_SESSION['trip-cost'] = $advance;
			$_SESSION['due'] = $tcost - $advance;
		}
		else{
			$_SESSION['due'] = '';
		}
	...
	<form method="post" id="wp-travel-engine-order-form" method="post" name="wp-travel-engine-order-form" action="<?php echo esc_url($wp_travel_engine_confirm);?>">
		...
		?>
			...
			<div id="wp-travel-engine-terms">
				...
				<?php
				printf( __( 'Check the box to confirm you\'ve read and agree to our <a href="%s" id="contact" target="_blank"> Terms and Conditions</a>.', 'wp-travel-engine'), esc_url( get_permalink( $wp_travel_engine_terms_conditions ) ) ); ?><span class="required">*</span>
				</label>
			</div>
			<?php } ?>
			<div class="error"></div>
