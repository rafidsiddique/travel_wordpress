				...
				<span class="handle"></span>
					<div class="form-builder">
						...
						<div class="field-icon">
							<label for="wp_travel_engine_settings[trip_facts][field_icon][{{tripfactsindex}}]"><?php _e('Field Icon:');?></label> 
	...
	function wp_travel_engine_activate_notice() {
	    global $current_user;
      	...
      	if (get_user_meta($user_id, 'wp-travel-engine-update-notice',true)!='true') {
      		$link_plugin = '<a href="https://wordpress.org/plugins/wp-travel-engine/" target="_blank">WP Travel Engine</a>';
      		$change_log = '<a href="https://wordpress.org/plugins/wp-travel-engine/#developers" target="_blank">Change Log</a>';
      		$link_site = '<a href="https://wptravelengine.com/downloads/category/add-ons/" target="_blank">here</a>';
			$message = sprintf( esc_html__( 'We have added some exciting features in %1$s. Go to %2$s Trips > Settings %3$s or see the %10$s for the added features. Also we have released %4$s Stripe %5$s and %6$s Authorize.net %7$s payment gateways for %8$s. Get them %9$s. ', 'wp-travel-engine' ), $link_plugin, '<b>', '</b>', '<b>', '</b>', '<b>', '</b>', $link_plugin, $link_site, $change_log );
